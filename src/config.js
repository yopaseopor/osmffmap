...
style: function (feature) {
		...
		{
			group: 'Emergency',
			title: 'Protección Civil (SES)',
			query: '(nwr["emergency"="ses_station"]({{bbox}});node(w););out meta;',
			iconSrc: imgSrc + 'icones/ses_logo.png',
			iconStyle: 'background-color:rgba(255,255,255,0.4)',
style: function (feature) {
				var key_regex = /^name$/
				var name_key = feature.getKeys().filter(function(t){return t.match(key_regex)}).pop() || "name"
				...
				var style = new ol.style.Style({
					image: new ol.style.Icon({
							src: imgSrc + 'icones/ses_logo.png',
							scale:0.02
						}),
							text: new ol.style.Text({
								text: name,
  ...
  },
		{
			...
			iconSrc: imgSrc + 'icones/hospital_logo.png',
			iconStyle: 'background-color:rgba(255,255,255,0.4)',
style: function (feature) {
				var key_regex = /^name$/
				var name_key = feature.getKeys().filter(function(t){return t.match(key_regex)}).pop() || "name"
				...
				var style = new ol.style.Style({
					image: new ol.style.Icon({
							src: imgSrc + 'icones/hospital_logo.png',
							scale:0.02
						}),
							text: new ol.style.Text({
								text: name,
  ...
  },
		{
			...
			iconSrc: imgSrc + 'icones/clinic_logo.svg',
			iconStyle: 'background-color:rgba(255,255,255,0.4)',
style: function (feature) {
				var key_regex = /^name$/
				var name_key = feature.getKeys().filter(function(t){return t.match(key_regex)}).pop() || "name"
...
//Mapcomplete editor
		...
		complete.append($('<a>').css('marginLeft', 5).attr({title: 'Mapcomplete hydrants', href: 'https://mapcomplete.org//hailhydrant.html?z=' + view.getZoom() +'&lat='+ coordinateLL[1] +'&lon='+ coordinateLL[0] +'&language=en&background=osm', target: '_blank'}).html($('<img>').attr({src: imgSrc + 'mapcomplete_logo.png', height: 20, width: 20})));
		complete.append($('<a>').css('marginLeft', 5).attr({title: 'Mapcomplete track type', href: 'https://mapcomplete.org/index.html?z=' + view.getZoom() +'&lat='+ coordinateLL[1] +'&lon='+ coordinateLL[0] +'&userlayout=https%3A%2F%2Fraw.githubusercontent.com%2Fyopaseopor%2Fmcquests%2Fmain%2Ftracktype.json&language=en#welcome', target: '_blank'}).html($('<img>').attr({src: imgSrc + 'icones/no_tracktype.svg', height: 20, width: 20})));
		...
		return $.merge($.merge($.merge(edit, open), complete), tool);
	},
